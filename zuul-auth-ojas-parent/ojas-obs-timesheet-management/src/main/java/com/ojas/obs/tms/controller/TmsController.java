		...
		try {
			...
			if (((request.getTransactionType().equalsIgnoreCase(GETBYID)
					...
					|| (request.getTransactionType().equalsIgnoreCase(GETBYPROJECT)
							&& (request.getSheet().getProjectId() == null || request.getSheet().getEmployeeId() == null || request.getSheet().getStartDate() == null
									|| request.getSheet().getEndDate() == null))) {
				ErrorResponse error = new ErrorResponse();
				error.setStatusCode("422");
	...
	private String validateRecords(TmsRequest request) throws IllegalAccessException {
		...
		for (Record rec : records) {
			for (Field f : rec.getClass().getDeclaredFields()) {
				f.setAccessible(true);
				if (f.getName().equalsIgnoreCase("sheet") || (request.getTransactionType().equalsIgnoreCase(SAVE)
						&& (f.getName().equalsIgnoreCase("recordId") || f.getName().equalsIgnoreCase("hoursLogged")
								|| f.getName().equalsIgnoreCase("hours")
								|| f.getName().equalsIgnoreCase("employeeId")))) {
					continue;
				}
